...
const CommonProvider = (props: React.PropsWithChildren<ICommonProviderProps>) => {
    ...
    const loadData = React.useCallback(async () => {
        const res = await api.agriculturalProduce.farmGarden.pack.list(numberOfRowsRedux, numberOfRows);
    ...
    const handlerBtnSearchClick = React.useCallback(() => {
        ...
        const filterData = resData.filter(item => {
            return processKeyword(item.name).toLowerCase().includes(processKeyword(searchInput).trim().toLowerCase())
        })
