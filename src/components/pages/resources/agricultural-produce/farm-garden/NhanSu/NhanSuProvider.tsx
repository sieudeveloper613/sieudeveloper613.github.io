...
export const NhanSuProvider = React.createContext<{
...
export default function DriverProvider(props: React.PropsWithChildren<IDriverProviderProps>) {
    ...
    const handlerBtnRemoveClick = React.useCallback(
        async (_id: string) => {
            messageBox({
                message: "Bạn chắc chắn muốn xóa nhân sự này?",
    ...
    const handlerBtnSaveClick = async () => {
        ...
        const name = preProcess.removeAllSpace(formData.name as string);

        if (nameAdded.has(name)) {
            messageAlert("warning", `Tên nhân sự này đã tồn tại.`);
            return;
        }
