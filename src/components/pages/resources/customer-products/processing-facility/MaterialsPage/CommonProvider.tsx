...
const CommonProvider = (props: React.PropsWithChildren<ICommonProviderProps>) => {
    ...
    const handlerBtnSaveClick = React.useCallback(async () => {
        ...
        if (!idUpdate) {
            ...
            if (name.length === 0 && code.length < 10) {
                messageAlert('warning', 'Bạn cần hoàn thiện một số mục trước khi lưu !');
                return;
            }

            if (nameAdded.has(name.toLowerCase())) {
                messageAlert('warning', `Bạn đã thêm ${Self.title.toLowerCase()} có tên là "${name}" này rồi !`);
                return;
            }
            if (suppliersDataSheet?.length < 1) {
                messageAlert('warning', `Bạn cần thêm tối thiểu một nhà cung cấp  !`);
                return;
            }
            await create({
                name,
    ...
    }, [idUpdate, nameAdded, create, update,suppliersDataSheet]);
