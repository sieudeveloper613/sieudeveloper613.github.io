...
import { ETypeEnterpriseProduct } from '../../../../../../sharetype/form-data/resources/enterprise/EnterpriseProductFormData/EnterpriseProductFormData';
...
export const CommonContext = React.createContext<{
    ...
    typeSupppliesOptions: IOption[];
    typeSuppliesSelected: string | undefined;
    setTypeSuppliesSelected: React.Dispatch<React.SetStateAction<string | undefined>>;
...
export default function CommonProvider(props: React.PropsWithChildren<ICommonProviderProps>) {
    ...
    const [typeSuppliesSelected, setTypeSuppliesSelected] = React.useState<string>();
    ...
    const DataTypeForProduce = [
        {
            name: 'Sản phẩm không cố định khối lượng',
            _id: ETypeEnterpriseProduct.fixed
        },
        {
            name: 'Sản phẩm cố định khối lượng',
            _id: ETypeEnterpriseProduct.noFixed
        },
    ]

    function returnType(item: any) {
        switch (item) {
            case 'FIXED':
                return 'Sản phẩm không cố định khối lượng';
            case 'NOFIXED':
                return 'Sản phẩm cố định khối lượng';
            default:
                return '';
        }
    ...
    const typeSupppliesOptions = useAsyncMemo(
        [],
        async () => {
            return DataTypeForProduce.map((item) => {
                return {
                    label: item.name,
                    value: item._id,
                } as IOption;
    ...
    const productOptions = useAsyncMemo(
        [],
        async () => {
            const res = await api.enterprise.enterpriseProducts.processingFacility.listAll();
            if (res.status === 'failure') {
                messageAlert('error', 'Không thể tải danh sách các Doanh nghiệp từ máy chủ !');
                return [];
            }
            function filterItem(array: any, type: string | undefined) {
                if (typeSuppliesSelected === undefined) { return [] }
                const temp = array?.length > 0 ? array.filter((e: any) => e.type == type) : []
                return temp?.length > 0 ? temp : {}
            }
            const resData = filterItem(res.data, typeSuppliesSelected)
    ...
    const supppliesOptions = useAsyncMemo(
        [],
        async () => {
            const res = await api.agriculturalProduce.farmGarden.ingredient.list(1, 500);
            if (res.status === 'failure') {
                messageAlert('error', 'Không thể tải danh sách các Doanh nghiệp từ máy chủ !');
                return [];
            }
    ...
    }, [suppliesSelected, supppliesData]);
    ...
    const unfilteredSuppliersDataSheet = useAsyncMemo(
        ...
        () => {
            ...
            if (res?.listIngredient?.length < 0) Object.freeze([]);

            return Object.freeze(
                res.listIngredient.map((item: any) => {
                    return {
                        _id: item._id,
                        items: [item.items],
                    } as ITableCell;
    ...
    const handlerBtnUpdateClick = React.useCallback((_id: string) => {
        ...
        setIdUpdate(_id);
        setTypeSuppliesSelected(res.type)
        setProductSelected(res.productName)
        setDisplayForm(true);
        setTimeout(() => {
            setCheckValidAll(true);
        }, 700);
        if (res?.listIngredient?.length < 0) {
            Object.freeze([])
        } else {
            res.listIngredient
                .forEach((item: any) => {
                    ...
                    res.listIngredient = res.listIngredient.splice(res.typeSupplies, 0)
                })
        };
    }, [resData]);
    ...
    const handlerBtnSaveClick = React.useCallback(async () => {
        let resultListIngredient: IListType[] = []
        let resultProductId
        setCheckValidAll(true);
        if (productSelected) {
        ...
        if (!idUpdate) {
            ...
            //     listIngredient: resultListIngredient
            // });
    ...
    const handlerBtnSearchClick = React.useCallback(() => {
        ...
        const filterData = [...resData].filter((item: any) => {
            return processKeyword(item.productName).toLowerCase().includes(processKeyword(searchInput).trim().toLowerCase())
                || item.gtin.toLowerCase().includes(searchInput.trim().toLowerCase())
        })
        if (filterData) {
    ...
    React.useEffect(() => {
        ...
        suppliersManager.reset();
        setIdUpdate(undefined);
        setTypeSuppliesSelected(undefined)
